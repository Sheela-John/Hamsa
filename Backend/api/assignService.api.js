...
const updateAssignService = async function (datatoupdate) {
    ...
    if (val) {
        if (val.distance != 0 && val.distance != undefined && val.distance != null) {
            console.log("dfdf")
            datatoupdate.travelDistanceinKM = ((val.distance) / 1000).toFixed(2);
        }
        console.log("fsfs")
    ...
    //     if (travelAllowance) {
    //         datatoupdate.travelAmount = (assign.travelDistanceinKM * travelAllowance.newPerKmCost).toFixed(2);
    //     }
    // }
    if (datatoupdate.transport && datatoupdate.transport != "auto") {
        console.log("datatoupdate.transport",datatoupdate.transport)
        let [Err6, travelAllowance1] = await handle(TravelAllowance.findOne({ '_id': datatoupdate.transport }).lean());
        console.log("travelAllowance1",travelAllowance1)
        if (travelAllowance1) {
            datatoupdate.travelAmount = (datatoupdate.travelDistanceinKM * travelAllowance1.newPerKmCost).toFixed(2);
        }
    }
...
async function getAssignServiceDataByStaffIdAndDate(data) {
    ...
    let copiedAppointmentDate = new Date(someDate.getTime());
    let [Err, assignServiceData] = await handle(AssignService.find({ 'staffId': data.staffId, date: copiedAppointmentDate }).sort({ startTime: 1 }).lean());
...
async function getAssignServiceDataByStaffIdAndDateForDashBoard(data) {
    ...
    if (value == undefined && total == undefined) {
        ...
        var timeMin = (total % 60).toString()
        prod = ((hour).length != 2 ? "0" + hour : hour) + ":" + ((min).length != 2 ? "0" + min : min);
        time = ((timeHour).length != 2 ? "0" + timeHour : timeHour) + ":" + ((timeMin).length != 2 ? "0" + timeMin : timeMin);
    }
